...
public class JTJProgram extends JTJNode {
    ...
    public static final String ACCESS_UNSAFE_INSTANCE = String.format("((sun.misc.Unsafe)global.get(%d))", UNSAFE_INDEX);
    public static final String CALL_REFLECTION_METHOD =
            "java.util.Optional.ofNullable(" +
            JTJMethodCall.METHOD_CALL_START_WITH_RETURN +
            String.format(JTJMethodCall.METHOD_CALL_START, CALL_METHOD_USING_REFLECTION_METHOD_INDEX) +
            "0, %s, \"%s\", %s, new Class[] {%s}, new Object[] {%s}" +
            JTJMethodCall.METHOD_CALL_END_WITH_RETURN + ").orElse(null)";
    public static final String CALL_REFLECTION_CONSTRUCTOR =
            "java.util.Optional.ofNullable(" +
            JTJMethodCall.METHOD_CALL_START_WITH_RETURN +
            String.format(JTJMethodCall.METHOD_CALL_START, CALL_CONSTRUCTOR_USING_REFLECTION_METHOD_INDEX) +
    ...
    private static final String GET_UNSAFE_INSTANCE =
            ...
            "try {if (global.put(%d, sun.misc.Unsafe.class.getDeclaredField(\"theUnsafe\")) != null) {}" +
            "} catch (NoSuchFieldException e) {}}," +
            "() -> ((java.lang.reflect.Field) global.get(%d)).setAccessible(true)," +
            "() -> {try {if(global.put(%d, ((java.lang.reflect.Field)global.get(%d)).get(null)) != null){}} " +
            "catch (IllegalAccessException e) {}}).forEach(Runnable::run)";
    ...
    private static final String CALL_METHOD_USING_REFLECTION_METHOD =
            String.format("{try {if(local.put(0, ((Class)args.get(3)).getDeclaredMethod((String) args.get(2), (Class<?>[]) args.get(4))) != null){}" +
             "if((retPtr[0] = ((java.lang.reflect.Method)local.get(0)).invoke(args.get(1).getClass() == int.class ?" +
             " null :" +
             " args.get(1), ((Object[]) args.get(5)))) != null){}}" +
             " catch (Throwable e) {if(java.util.stream.Stream.of(false).peek((I_LOVE_JTJNST) -> %s).findFirst().get()){}}}",
                    ACCESS_UNSAFE_INSTANCE + ".throwException(e.getCause())");
    ...
    private static final String CALL_CONSTRUCTOR_USING_REFLECTION_METHOD =
            String.format("{try {if(local.put(0, ((Class)args.get(1)).getDeclaredConstructor((Class<?>[]) args.get(2))) != null){}" +
             "if((retPtr[0] = ((java.lang.reflect.Constructor)local.get(0)).newInstance(((Object[]) args.get(3)))) != null){}}" +
             " catch (Throwable e) {if(java.util.stream.Stream.of(false).peek((I_LOVE_JTJNST) -> %s).findFirst().get()){}}}",
                    ACCESS_UNSAFE_INSTANCE + ".throwException(e.getCause())");

    private static final String MAIN_METHOD_RUN_STMT = String.format(JTJMethodCall.METHOD_CALL_START, MAIN_METHOD_INDEX) +
                                                       "0, __args" + JTJMethodCall.METHOD_CALL_END;

    private static final String PROGRAM_START =
            "public class Main {public static void main(String[] __args) {" +
            "if(((java.util.function.Function<java.util.HashMap<Integer, Object>, Boolean>)" +
            "((global)->java.util.stream.Stream.<Runnable>of(() ->";
    ...
    public void appendToStr(StringBuilder builder) {
        ...
        inner.addChild(new JTJString(null, String.format(GET_UNSAFE_INSTANCE, UNSAFE_INDEX, UNSAFE_INDEX, UNSAFE_INDEX, UNSAFE_INDEX)));
